...
namespace Heralabs.ClientManagement.Api.Controllers
{
    ...
    public class ClientsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetClientInfo(string clientCode, [FromQuery] string mobileAppVersion, [FromQuery] string mobilePlatform)
        {
            try
            {
                _logger.LogInformation($"Init request for client: {clientCode}");

                var clientInfo = await _clientCacheService.GetClientInfoAsync(clientCode);
                if (clientInfo == null)
                {
                ...
                {
                    if (!string.IsNullOrEmpty(mobileAppVersion) &&
                        !string.IsNullOrEmpty(mobilePlatform) &&
                        !string.IsNullOrEmpty(clientInfo.WebApiVersionCode))
                    {
                        ...
                        {
                            long currentAppVerOrder = VersionHelper.CalculateVersionOrder(mobileAppVersion);
                            ...
                            {
                                _logger.LogWarning($"Invalid version format passed. ClientVer: {mobileAppVersion}");
                            }
                        }
                    }
                }
            ...
            {
                _logger.LogError($"GetClientInfo '{clientCode}' Exception: {ex.Message}", ex);
